...
const columns = reactive<ColumnProps<any>[]>([
  ...
  { prop: "sourceDepotName", label: "来源仓库", search: { el: "input" }, width: 160 },
  { prop: "sourceLocationNo", label: "来源库位", width: 160 },
  { prop: "targetDepotName", label: "目标仓库", search: { el: "input" }, width: 160 },
...
const downloadFile = async () => {
  ElMessageBox.confirm("确认导出数据?", "温馨提示", { type: "warning" }).then(() => {
    ...
    if (proTable.value?.searchParam.createTime.length) {
      ...
      useDownload(exportDepotRecord, "转库记录列表", {
        operateType: 3,
        startDate: startDate,
        endDate: endDate,
      ...
      useDownload(exportDepotRecord, "转库记录列表", { operateType: 3, ...proTable.value?.searchParam });
    }
  });
};
